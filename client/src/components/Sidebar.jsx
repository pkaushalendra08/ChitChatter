import { useNavigate } from 'react-router-dom'
import logo from '../assets/logo.png'
import { MdMenu, MdOutlineSearch } from "react-icons/md";
import { FiUser } from "react-icons/fi";
import { useContext, useEffect, useState } from 'react';
import { AuthContext } from '../../context/AuthContext'
import { ChatContext } from '../../context/ChatContext';

const Sidebar = () => {

  const { getUsers, users, selectedUser, setSelectedUser, unseenMessages, setUnseenMessages } = useContext(ChatContext)

  const { logout, onlineUsers, authUser } = useContext(AuthContext)

  const [menuOpen, setMenuOpen] = useState(false);

  const [input, setInput] = useState("");

  const navigate = useNavigate();



  if (!authUser) {
    return (
      <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl text-white`}>
        <p>Loading...</p>
      </div>
    )
  }

  const filteredUsers = input ? users.filter((user) => user.fullName.toLowerCase().includes(input.toLowerCase())) : users;

  useEffect(() => {
    getUsers();
  }, [onlineUsers, authUser, getUsers])

  return (
    <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? 'max-md:hidden' : ''}`}>
      <div className='pb-5'>
        {/* heading */}
        <div className='flex justify-between items-center'>
          <img src={logo} alt="logo" className='max-w-40' />
          <div className='relative py-2'>

            <div onClick={() => setMenuOpen(!menuOpen)}
              className='flex items-center gap-2 cursor-pointer'>
              <span>Hi, {authUser.fullName}</span>
              <MdMenu size={28} />
            </div>

            {menuOpen && (
              <div className='absolute top-full right-0 z-20 w-32 p-5 rounded-md bg-[#282142] border border-gray-600 text-gray-100 group-hover:block'>
                <p onClick={() => navigate('/profile')} className='cursor-pointer text-sm'>
                  Edit Profile
                </p>
                <hr className='my-2 border-t border-gray-500' />
                <p onClick={() => logout()}
                  className='cursor-pointer test-sm'>
                  Logout
                </p>
              </div>
            )}
          </div>
        </div>

        {/* search  */}
        <div className='bg-[#282142] rounded-full flex items-center gap-2 py-3 px-4 mt-5'>
          <MdOutlineSearch size={28} className='' />
          <input type="text" onChange={(e) => setInput(e.target.value)}
            className='bg-transparent border-none outline-none text-white text-xs 
          placeholder-[#c8c8c8] flex-1' placeholder='Search User....' />
        </div>
      </div>

      {/* chat list */}
      <div className='flex flex-col'>
        {filteredUsers.map((user, index) => (
          <div
            onClick={() => {
              setSelectedUser(user);
              if (unseenMessages[user._id]) {
                setUnseenMessages(prevUnseen => ({
                  ...prevUnseen,
                  [user._id]: 0
                }));
              }
            }}
            key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm 
              ${selectedUser?._id === user._id && 'bg-[#282142]/50'}`}>
            {user?.profilePic ? (
              <img src={user.profilePic} className='w-[35px] h-[35px] aspect-[1/1] rounded-full object-cover' />
            ) : (
              <FiUser size={28} className='w-[35px] h-[35px] rounded-full' />
            )}
            <div>
              <p>{user.fullName}</p>
              {
                onlineUsers.includes(user._id)
                  ? <span className='text-green-400 text-xs'>Online</span>
                  : <span className='text-neutral-400 text-xs'>Offline</span>
              }
            </div>
            {
              unseenMessages[user._id] > 0 && <p className='absolute top-4 right-4 text-xs h-5 w-5 
                flex justify-center items-center rounded-full bg-violet-500/50 '>{unseenMessages[user._id]}</p>
            }
          </div>
        ))}
      </div>
    </div>
  )
}

export default Sidebar