...
const ChatContainer = () => {
  ...
  return selectedUser ? (
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
      ...
      <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
        {messages.map((msg, index) => (
          <div key={index}
            className={`flex item-end gap-2 justify-end ${msg.senderId !== authUser._id && 'flex-row-reverse'}`}>
            {msg.image ? (
            ...
            <div className='text-center text-xs'>
              ...
              />
              <p className='text-black-500'>{formatMessageTime(msg.createdAt)}</p>
            </div>
          </div>
        ))}
    ...
    <div className='flex flex-col items-center justify-center gap-2 text-grey-500 bg-white/10 max-md:hidden'>
      <img src={assets.logo} alt="" className='max-w-50' />
